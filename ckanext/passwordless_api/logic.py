...
def request_api_token_azure_ad(
    ...
    To be called from redirectUri page on frontend.
    Must be configured in the plugin settings:
        - azure_ad_tenant_id
        - azure_ad_client_id
    ...
    Args:
        data_dict.email (str): Email of user.
        data_dict.auth_code (str): Token generated by authorization code
            flow in Azure AD frontend login.
        data_dict.redirect_uri (str): The redirect uri used in the frontend Azure AD
            login flow (must match).
        data_dict.state (str): The state generated from the frontend Azure AD
            login flow (must match).

    Returns:
        dict: CKAN API token for user {'token': token_value}.
    """
    log.debug(f"Requesting API token using Azure AD auth code with params: {data_dict}")

    # Check if parameters are present
    if not (email := data_dict.get("email")):
        raise toolkit.ValidationError({"email": "missing email"})
    if not (auth_code := data_dict.get("auth_code")):
        raise toolkit.ValidationError({"auth_code": "missing authorization code"})
    if not (redirect_uri := data_dict.get("redirect_uri")):
        raise toolkit.ValidationError({"redirect_uri": "missing redirect uri"})
    if not (state := data_dict.get("state")):
        raise toolkit.ValidationError({"state": "missing state param from login flow"})

    log.debug("Attempting to import requests package")
    try:
        from requests import post
    except ImportError as e:
        log.error(e)
        raise ImportError(
            "This endpoint requires the `requests` dependency to be installed. "
            "Try installing with `pip install requests`."
        )
    ...
    azure_ad_client_secret = config.get("passwordless_api.azure_ad_client_secret", None)
    if None in (azure_ad_tenant_id, azure_ad_client_id, azure_ad_client_secret):
        err_str = (
            "passwordless_api.azure_ad_xxx settings are required to use this endpoint"
        )
        log.error(err_str)
    ...
    try:
        url = (
            ...
            f"{azure_ad_tenant_id}/oauth2/v2.0/token"
        )
        params = {
        ...
        log.debug(f"Requesting token from url {url} with params {params}")
        response = post(url, data=params)
        log.debug(response.content)

        access_token = response.json().get("access_token", None)
        if not access_token:
            raise toolkit.NotAuthorized("the login attempt was unsuccessful")

    except Exception as e:
        log.error(e)
        raise toolkit.ValidationError({"auth_code": "unknown error during validation"})
    # if not payload:
    #     log.error("Azure AD JWT validation failed")
    #     raise toolkit.NotAuthorized({"message": "Azure AD JWT validation failed"})
    ...
    user_id = user.id
    log.debug(f"User id: {user_id} {auth_code}")
